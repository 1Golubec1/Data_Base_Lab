...
orders_bound_to_contract AS (
    ...
    FROM contract c
    LEFT JOIN taxi_order o ON o.id_driver = c.id_user AND o.date_clock::date >= c.date_start
       AND (c.date_end IS NULL OR o.date_clock::date <= c.date_end)
...
SELECT
    ...
    COUNT(obc.order_id) FILTER (
        WHERE obc.status = 'completed'
    ) AS "Число выполненных заказов по договору",
    COUNT(obc.order_id) FILTER (
        WHERE obc.id_refusal IS NOT NULL
    ) AS "Число отказов по договору",
    COALESCE(SUM(obc.current_cost) FILTER (
        WHERE obc.status = 'completed'
    ), 0) AS "Сумма заказов по договору",
    COALESCE(SUM(obc.driver_payment), 0) AS "Сумма платежей водителю"
FROM contract_base cb
LEFT JOIN orders_bound_to_contract obc
    ON obc.contract_id = cb.contract_id
GROUP BY
    cb.contract_id,
    cb.driver_name,
    cb.car_name,
